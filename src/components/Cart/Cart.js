...
const Cart = (props) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8000/api/v1/carts/deletes/${user._id}`, cartIds)
      ...
      let orderResult = await axios.post(`http://localhost:8000/api/v1/orders`, shippingInfo)
      setIsLoading(false)
